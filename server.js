...
// Utility function to check if any transactions exist
const hasTransactions = async () => await Transaction.exists();

// Bar chart API
app.get('/bar-chart', async (req, res) => {
  const { month } = req.query;

  if (!await hasTransactions()) {
    return res.json([]); // Return empty array if no transactions exist
  }
...
app.get('/statistics', async (req, res) => {
  ...
  if (!await hasTransactions()) {
    return res.json({}); // Return empty object if no transactions exist
  }
